    ...
    def test(self, arguments):
        ...
        if len(arguments) != 1:
            ...
            if arguments[1] == "--ingredients=milk,sugar":
                ...
                if any(["Milkshake" not in output, "Fruit salad" not in output, "cacao" not in output]):
                    return CheckResult.wrong(f"Wrong answer. Founded '{output}', expected Milkshake, Hot cacao and Fruit Salad.")
                ...
                if "no such recipes" not in output:
                    pass#return CheckResult.wrong(f"Wrong answer. Founded '{output}', expected 'no such recipes'.")
